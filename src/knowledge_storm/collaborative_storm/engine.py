...
from ..lm import OllamaClient, GroqModel, OpenAIModel
...
class CollaborativeStormLMConfigs(LMConfigs):
    ...
    def init(
            self,
            lm_type: Literal["openai", "groq", "ollama"],
        ...
        if lm_type == "openai":
            ...
            self.question_answering_lm = GroqModel(
                model=groq_model, max_tokens=1000, **groq_kwargs
            )
            self.discourse_manage_lm = GroqModel(
                model=groq_model, max_tokens=500, **groq_kwargs
            )
            self.utterance_polishing_lm = GroqModel(
                model=groq_model, max_tokens=2000, **groq_kwargs
            )
            self.warmstart_outline_gen_lm = GroqModel(
                model=groq_model, max_tokens=500, **groq_kwargs
            )
            self.question_asking_lm = GroqModel(
                model=groq_model, max_tokens=300, **groq_kwargs
            )
            self.knowledge_base_lm = GroqModel(
                model=groq_model, max_tokens=1000, **groq_kwargs
            )
        elif lm_type == "ollama":
            ollama_kwargs = {
                # Add any Ollama-related options here if needed
            }
            ollama_model = "llama3"
            ollama_port = int(os.getenv("OLLAMA_PORT", "11434"))
            ollama_url = os.getenv("OLLAMA_URL", "http://localhost")
            self.question_answering_lm = OllamaClient(
                model=ollama_model, port=ollama_port, url=ollama_url, max_tokens=1000, **ollama_kwargs
            )
            self.discourse_manage_lm = OllamaClient(
                model=ollama_model, port=ollama_port, url=ollama_url, max_tokens=500, **ollama_kwargs
            )
            self.utterance_polishing_lm = OllamaClient(
                model=ollama_model, port=ollama_port, url=ollama_url, max_tokens=2000, **ollama_kwargs
            )
            self.warmstart_outline_gen_lm = OllamaClient(
                model=ollama_model, port=ollama_port, url=ollama_url, max_tokens=500, **ollama_kwargs
            )
            self.question_asking_lm = OllamaClient(
                model=ollama_model, port=ollama_port, url=ollama_url, max_tokens=300, **ollama_kwargs
            )
            self.knowledge_base_lm = OllamaClient(
                model=ollama_model, port=ollama_port, url=ollama_url, max_tokens=1000, **ollama_kwargs
            )
        else:
            raise Exception(
                "No valid LLM provider is provided. Supported: openai, groq, ollama."
            )
...
class CoStormRunner:
    def __init__(
        ...
        self.discourse_manager = DiscourseManager(
            ...
            logging_wrapper=self.logging_wrapper,
            rm=self.rm,
            encoder=self.encoder,  # Pass the same singleton
    ...
    def from_dict(cls, data, callback_handler: BaseCallbackHandler = None):
        ...
        costorm_runner.encoder = get_encoder()  # Use the singleton encoder
    ...
    def step(
        ...
        """
        last_conv_turn = self.conversation_history[-1] if self.conversation_history else None
        ...
        with self.logging_wrapper.log_pipeline_stage(
            ...
            if user_utterance:
                ...
                if turn_policy.should_update_experts_list and last_conv_turn is not None:
                    with self.logging_wrapper.log_event(
