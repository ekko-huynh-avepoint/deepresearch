...
from ...encoder import get_encoder  # Use the singleton getter
from ...interface import Information


class InsertInformation(dspy.Signature):
    """Your job is to insert the given information to the knowledge base. The knowledge base is a tree based data structure to organize the collection information. Each knowledge node contains information derived from thematically similar question or intent.
    To decide the best placement of the information, you will be navigated in this tree based data structure layer by layer.
    You will be presented with the question and query leads to thesis information, and tree structure.
...
class InsertInformationModule(dspy.Module):
    def __init__(self, engine: Union[dspy.dsp.LM, dspy.dsp.HFModel], encoder: Optional[object] = None):
        self.engine = engine
...
class ExpandSection(dspy.Signature):
    ...
    Output should be subsection names where each section should serve as a coherent and thematic organization of information and corresponding citation numbers. These subsection names are preferred to be concise and precise.
