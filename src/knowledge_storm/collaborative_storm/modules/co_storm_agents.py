...
class Moderator(Agent):
    ...
    This is achieved by generating questions based on unused, united snippets of information retrieved since the last moderator's turn.
    ...
    def __init__(
        ...
        runner_argument: "RunnerArgument",
        logging_wrapper: LoggingWrapper,
        encoder=None,  # allow encoder to be optional for mock/test, but default to None
        callback_handler: BaseCallbackHandler = None,
    ):
        super().__init__(topic, role_name, role_description)
        self.lm_config = lm_config
        ...
        self.callback_handler = callback_handler
        # Use the singleton encoder if not explicitly provided
    ...
    def generate_utterance(
        ...
        with self.logging_wrapper.log_event(
            "Moderator generate utterance: get unused snippets"
        ):
        ...
        with self.logging_wrapper.log_event(
            "Moderator generate utterance: QuestionGeneration module"
        ):
...
class PureRAGAgent(Agent):
    ...
    def generate_utterance(
        ...
        with self.logging_wrapper.log_event(
            "PureRAGAgent generate utterance: generate utterance"
        ):
